...
int main(int argc, char* argv[]) {
    ...
    FILE* file = fopen("./assets/sprite_assets.h", "wb");
    const char* includes_section = "//This file was autogenerated via game_tools/asset_manager\n"
        "#include \"waffle_lib.h\"\n\n";

    fwrite(includes_section, w_str_len(includes_section), sizeof(char), file);

    const char* sprite_enum_declaration = "enum SpriteID {\n";
    fwrite(sprite_enum_declaration, w_str_len(sprite_enum_declaration), sizeof(char), file);
    ...
    const char* enum_close = "\tSPRITE_COUNT\n};\n\n";
    ...
    const char* anim_enum_definition = "enum AnimationID {\n";
    fwrite(anim_enum_definition, w_str_len(anim_enum_definition), sizeof(char), file);

    for (int i = 0; i < anim_count; i++) {
        AnimationExtraction* anim_ext = &animation_extracts[i];
        char anim_enum_line[128] = {};
        snprintf(anim_enum_line, 128, "\t%s,\n", anim_ext->animation_label);
        fwrite(anim_enum_line, w_str_len(anim_enum_line), sizeof(char), file);
    }

    const char* anim_enum_close = "\tANIM_COUNT,\n};\n\n";
    fwrite(anim_enum_close, w_str_len(anim_enum_close), sizeof(char), file);
