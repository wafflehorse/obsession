...
ASSET_BITMAP_OUTPUT_DIR="./assets/bitmaps_gen"
ASSET_DATA_FILE_OUTPUT_DIR="./assets/aseprite_data"


rm -rf $ASSET_BITMAP_OUTPUT_DIR
rm -rf $ASSET_DATA_FILE_OUTPUT_DIR
...
for filepath in $ASSET_SOURCE_DIR/*_frames.aseprite; do
	filename_with_ext="${filepath##*/}"
	filename_no_meta="${filename_with_ext%_frames.*}"
	filename_no_ext="${filename_with_ext%.*}"
	filename_no_ext_lowercase=$(echo "$filename_no_ext" | tr '[:upper:]' '[:lower:]')
	filename_no_meta_lowercase=$(echo "$filename_no_meta" | tr '[:upper:]' '[:lower:]')

	echo "processing frames: $filename_with_ext..."
